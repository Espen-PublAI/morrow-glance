...
Plugins that need credentials add a `server.ts` beside their `plugin.tsx`. It runs only in Morrow Server, reads secrets from the environment, and returns the data the views get. The **Calendar** plugin uses this to read Microsoft 365 calendars, either from a published Outlook link stored as a per-block secret, or live through Microsoft Graph with application permissions; see `plugins/calendar/README.md`. The **GitHub** plugin does the same for the commit activity of a repository or a whole organisation across every contributor, a repository's stars and open work, or one person's contributions and recent events. See `plugins/github/README.md`.
